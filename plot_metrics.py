...
COLORS_R2 = {'loss': '#f6c398', 'dice': '#a8d5e5', 'iou': '#b4e3b2', 'hd': '#d7bce8'}
COLORS_R3 = {'loss': '#dc3545', 'dice': '#0d6efd', 'iou': '#198754', 'hd': '#6f42c1'}
...
def parse_log_file(filepath, hd_pattern_type='v2'):
    ...
    if hd_pattern_type == 'v1':
        ...
        # Standard pattern supporting class Dice printing fallback too
        pattern = re.compile(
...
def plot_overlay_curves(r1_data, r2_data, r3_data, title, output_path):
    """
    Plots 4-panel learning curves with Run 1 (dashed), Run 2 (dash-dot), and Run 3 (solid) overlaid.
    HD panel uses dual y-axes since Run 1 is in px, and Runs 2/3 are in mm.
    """
    has_r1 = r1_data is not None and len(r1_data[0]) > 0
    has_r2 = r2_data is not None and len(r2_data[0]) > 0
    has_r3 = r3_data is not None and len(r3_data[0]) > 0
    ...
    ax = axes[0, 0]
    if has_r1:
        ax.plot(r1_data[0], r1_data[1], color=COLORS_R1['loss'], linewidth=1.5,
                linestyle='--', marker='o', markersize=3, alpha=0.7,
                label='Run 1 (ch=32, 8.57M)')
    if has_r2:
        ax.plot(r2_data[0], r2_data[1], color=COLORS_R2['loss'], linewidth=1.5,
                linestyle='-.', marker='s', markersize=3, alpha=0.7,
                label='Run 2 (ch=42, 14.5M, Faulty)')
    if has_r3:
        ax.plot(r3_data[0], r3_data[1], color=COLORS_R3['loss'], linewidth=2.5,
                marker='^', markersize=4,
                label='Run 3 (ch=42, 14.5M, Corrected)')
    ...
    ax = axes[0, 1]
    if has_r1:
        ax.plot(r1_data[0], r1_data[2], color=COLORS_R1['dice'], linewidth=1.5,
                linestyle='--', marker='o', markersize=3, alpha=0.7,
                label='Run 1 (ch=32, 8.57M)')
    if has_r2:
        ax.plot(r2_data[0], r2_data[2], color=COLORS_R2['dice'], linewidth=1.5,
                linestyle='-.', marker='s', markersize=3, alpha=0.7,
                label='Run 2 (ch=42, 14.5M, Faulty)')
    if has_r3:
        ax.plot(r3_data[0], r3_data[2], color=COLORS_R3['dice'], linewidth=2.5,
                marker='^', markersize=4,
                label='Run 3 (ch=42, 14.5M, Corrected)')
    ...
    ax = axes[1, 0]
    if has_r1:
        ax.plot(r1_data[0], r1_data[3], color=COLORS_R1['iou'], linewidth=1.5,
                linestyle='--', marker='o', markersize=3, alpha=0.7,
                label='Run 1 (ch=32, 8.57M)')
    if has_r2:
        ax.plot(r2_data[0], r2_data[3], color=COLORS_R2['iou'], linewidth=1.5,
                linestyle='-.', marker='s', markersize=3, alpha=0.7,
                label='Run 2 (ch=42, 14.5M, Faulty)')
    if has_r3:
        ax.plot(r3_data[0], r3_data[3], color=COLORS_R3['iou'], linewidth=2.5,
                marker='^', markersize=4,
                label='Run 3 (ch=42, 14.5M, Corrected)')
    ...
    if has_r1:
        l1, = ax.plot(r1_data[0], r1_data[4], color=COLORS_R1['hd'], linewidth=1.5,
                       linestyle='--', marker='o', markersize=3, alpha=0.7)
    ...
    if has_r2 or has_r3:
        ...
        if has_r2:
            l2, = ax2.plot(r2_data[0], r2_data[4], color=COLORS_R2['hd'], linewidth=1.5,
                            linestyle='-.', marker='s', markersize=3, alpha=0.7)
            lines.append(l2)
            labels.append('Run 2 HD (3D mm, Faulty)')
            
        if has_r3:
            l3, = ax2.plot(r3_data[0], r3_data[4], color=COLORS_R3['hd'], linewidth=2.5,
                            marker='^', markersize=4)
            lines.append(l3)
            labels.append('Run 3 HD (3D mm, Corrected)')
            
        ax2.set_ylabel('HD — Runs 2 & 3 (3D-HD95, mm)', color=COLORS_R3['hd'], weight='semibold')
        ax2.tick_params(axis='y', labelcolor=COLORS_R3['hd'])
...
def plot_comparison_chart(results, output_path):
    """
    Plots a grouped bar chart comparing Run 1 vs Run 2 vs Run 3 performance across datasets.
    """
    datasets = list(results.keys())
    
    fig, ax1 = plt.subplots(figsize=(12, 7))
    fig.suptitle("Dataset Performance Comparison — Run 1 vs Run 2 vs Run 3", weight='bold', y=0.98)
    ...
    r3_dices = [results[d].get('r3_dice', 0) for d in datasets]
    r3_ious = [results[d].get('r3_iou', 0) for d in datasets]
    
    # Bars
    ax1.bar(x - 2.5*width, r1_dices, width, label='Run 1 Dice', color='#c2dbff', edgecolor='#0d6efd', linewidth=0.5)
    ax1.bar(x - 1.5*width, r1_ious, width, label='Run 1 IoU', color='#d1f2d9', edgecolor='#198754', linewidth=0.5)
    ax1.bar(x - 0.5*width, r2_dices, width, label='Run 2 Dice', color='#6eaafc', edgecolor='#0d6efd', linewidth=0.5)
    ax1.bar(x + 0.5*width, r2_ious, width, label='Run 2 IoU', color='#5fbf8a', edgecolor='#198754', linewidth=0.5)
    ax1.bar(x + 1.5*width, r3_dices, width, label='Run 3 Dice', color='#0d6efd')
    ax1.bar(x + 2.5*width, r3_ious, width, label='Run 3 IoU', color='#198754')
    ...
    # Annotate each bar
    for bars in [ax1.containers[i] for i in range(6)]:
        for rect in bars:
            h = rect.get_height()
            if h > 0:
                ax1.annotate(f'{h:.4f}',
    ...
    ax1.legend(loc='lower right', fontsize=8.5, ncol=3)
...
def main():
    ...
    # Dataset configs: (display_name, run1_log, run2_log, run3_log, output_png)
    datasets = [
        ('Mendeley Lumbar MRI',
    ...
    for name, r1_log, r2_log, r3_log, out_png in datasets:
        ...
        # Parse Run 3 (new format: Val 3D-HD95: X.XX mm)
        r3 = parse_log_file(r3_log, hd_pattern_type='v2')
        
        r1_data = r1 if r1[0] else None
        r2_data = r2 if r2[0] else None
        r3_data = r3 if r3[0] else None
        
        # Plot overlay curves
        plot_overlay_curves(r1_data, r2_data, r3_data, name, out_png)
        ...
        if r3_data:
            best_idx = np.argmax(r3_data[2])
            entry['r3_dice'] = r3_data[2][best_idx]
            entry['r3_iou'] = r3_data[3][best_idx]
